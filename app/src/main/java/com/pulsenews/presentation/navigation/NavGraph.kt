...
fun NavGraph() {
    ...
    NavHost(navController = navController, startDestination = Screen.Subscription.route) {
        composable(Screen.Subscription.route) {
            SubscriptionScreen(onNavigationToSettings = { navController.navigate(Screen.Settings.route) }, onOpenArticle = { url -> navController.navigate("article/${URLEncoder.encode(url, StandardCharsets.UTF_8.toString())}") })
        }
